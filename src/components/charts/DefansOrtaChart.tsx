...
interface DefansOrtaChartProps {
...
export default function DefansOrtaChart({ 
  defansData, 
  isExpanded = false, 
  onToggleExpand,
  currentPlayerId 
}: DefansOrtaChartProps) {
  ...
  const referenceLinePlugin = useMemo(() => {
    return {
      id: 'referenceLines-orta-defanslar',
      afterDatasetsDraw(chart: any) {
        const { ctx, chartArea, scales } = chart;
        if (!chartArea || !scales.x || !scales.y) return;
        
        const { top, bottom, left, right } = chartArea;
        const yPos = scales.y.getPixelForValue(3.25);
        const xPos = scales.x.getPixelForValue(18);
  ...
  const chartData = useMemo(() => {
    const points: PointData[] = defansData
      ...
      .map(defans => ({
        x: safeNumber(defans["Ao-Io%"]),
        y: safeNumber(defans["OrtGrsm/90"]),
    ...
    return {
      ...
      scales: {
        ...
        y: {
          title: {
            display: isExpanded,
            text: 'Orta Girişimleri (90 Dakika Başına)',
            font: { size: 12 }
          },
          min: 0,
          max: 8,
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function(context: any) {
              return [
                `İsabetli Orta: ${context.parsed.x.toFixed(1)}%`,
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Orta-Defanslar</h3>
        {onToggleExpand && (
