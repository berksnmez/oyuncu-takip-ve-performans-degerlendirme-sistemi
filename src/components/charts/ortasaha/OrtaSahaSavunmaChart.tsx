...
interface OrtaSahaSavunmaChartProps {
...
const OrtaSahaSavunmaChart: React.FC<OrtaSahaSavunmaChartProps> = ({
  ortaSahaData,
  currentPlayerId,
  isExpanded,
  onToggleExpand
  ...
  const referenceLinePlugin = useMemo(() => {
    return {
      id: 'referenceLines-savunma-ortasaha',
      afterDatasetsDraw(chart: any) {
        ...
        const xPos = scales.x.getPixelForValue(1.3);
  ...
  const chartData = useMemo(() => {
    const points: PointData[] = ortaSahaData
      .filter(ortaSaha => {
        const engValue = safeNumber(ortaSaha["Eng/90"]);
        return engValue !== 0 && !isNaN(engValue) && engValue !== null;
      })
      .map(ortaSaha => ({
        x: safeNumber(ortaSaha["Uzk/90"]),
        y: safeNumber(ortaSaha["Eng/90"]),
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Savunma-Orta Sahalar</h3>
        {onToggleExpand && (
          <button 
            onClick={onToggleExpand}
...
export default OrtaSahaSavunmaChart; 