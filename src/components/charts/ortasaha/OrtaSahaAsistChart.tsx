...
interface OrtaSahaAsistChartProps {
...
const OrtaSahaAsistChart: React.FC<OrtaSahaAsistChartProps> = ({
  ortaSahaData,
  currentPlayerId,
  isExpanded,
  onToggleExpand
  ...
  const referenceLinePlugin = useMemo(() => {
    return {
      id: 'referenceLines-asist-ortasaha',
      afterDatasetsDraw(chart: any) {
        ...
        const xPos = scales.x.getPixelForValue(14.5);
  ...
  const chartData = useMemo(() => {
    const points: PointData[] = ortaSahaData
      .filter(ortaSaha => {
        const xAValue = safeNumber(ortaSaha["xA/90"]);
        return xAValue !== 0 && !isNaN(xAValue) && xAValue !== null;
      })
      .map(ortaSaha => ({
        x: safeNumber(ortaSaha["SPasi/90"]),
        y: safeNumber(ortaSaha["xA/90"]),
    ...
    return {
      ...
      scales: {
        ...
        y: {
          title: {
            display: isExpanded,
            text: 'Maç Başına Beklenen Asist',
            font: {
              size: 12
            }
          },
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function(context: any) {
              return [
                `Gol Asisti/90: ${context.parsed.x.toFixed(2)}`,
                `xA/90: ${context.parsed.y.toFixed(2)}`
              ];
            }
          }
        },
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Asist-Orta Sahalar</h3>
        {onToggleExpand && (
          <button
            onClick={onToggleExpand}
...
export default OrtaSahaAsistChart; 