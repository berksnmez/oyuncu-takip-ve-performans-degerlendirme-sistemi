...
interface OrtaSahaPasChartProps {
...
const OrtaSahaPasChart: React.FC<OrtaSahaPasChartProps> = ({
  ortaSahaData,
  currentPlayerId,
  isExpanded,
  onToggleExpand
  ...
  const referenceLinePlugin = useMemo(() => {
    return {
      id: 'referenceLines-pas-ortasaha',
      afterDatasetsDraw(chart: any) {
        ...
        const yPos = scales.y.getPixelForValue(83);
        ...
        if (isExpanded) {
          ...
          ctx.font = '12px Arial';
          ctx.textAlign = 'left';
          ctx.fillText('83%', right + 5, yPos + 4);
        }
        
        // Dikey çizgi için (ortalama değer)
        const xPos = scales.x.getPixelForValue(27);
  ...
  const chartData = useMemo(() => {
    const points: PointData[] = ortaSahaData
      .filter(ortaSaha => {
        const pasValue = safeNumber(ortaSaha["Pas%"]);
        return pasValue !== 0 && !isNaN(pasValue) && pasValue !== null;
      })
      .map(ortaSaha => ({
        x: safeNumber(ortaSaha["PsG/90"]),
        y: safeNumber(ortaSaha["Pas%"]),
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function(context: any) {
              return [
                `Pas Girişimi/90: ${context.parsed.x.toFixed(1)}`,
                `Pas İsabet %: ${context.parsed.y.toFixed(1)}%`
              ];
            }
          }
        },
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Pas-Orta Sahalar</h3>
        {onToggleExpand && (
          <button
            onClick={onToggleExpand}
      ...
      <div className="p-2">
        {isExpanded ? (
          <div className="flex flex-col md:flex-row">
            <div className="md:w-1/4 p-2">
              ...
              <p className="text-xs">Yüksek pas girişimi ve isabet oranı, oyun kurucu orta saha oyuncularını gösterir.</p>
            </div>
        ...
        <>
          ...
          <div className="absolute ml-12 mt-3 text-left z-10">
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Az Pas Yapar</div>
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>İsabetli Pas</div>
          </div>
          
          {/* Sağ üst köşe - Yeşil */}
          <div className="absolute right-0 mr-3 mt-3 text-right z-10">
            <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>Çok Pas Yapar</div>
            <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>İsabetli Pas</div>
          </div>
          
          {/* Sol alt köşe - Kırmızı */}
          <div className="absolute bottom-0 ml-12 mb-12 text-left z-10">
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>Az Pas Yapar</div>
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>İsabetsiz Pas</div>
          </div>
          
          {/* Sağ alt köşe - Turuncu */}
          <div className="absolute bottom-0 right-0 mr-3 mb-12 text-right z-10">
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Çok Pas Yapar</div>
            <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>İsabetsiz Pas</div>
          </div>
        </>
...
export default OrtaSahaPasChart; 