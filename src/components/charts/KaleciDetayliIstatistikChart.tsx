...
interface KaleciDetayliIstatistikChartProps {
...
export default function KaleciDetayliIstatistikChart({ 
  kaleciData, 
  highlightedPlayerId,
  title = "Detaylı Kalecilik İstatistikleri-Kaleciler",
  showExpandButton = true
}: KaleciDetayliIstatistikChartProps) {
  ...
  const referenceLinePlugin = useMemo(() => {
    return {
      ...
      afterDatasetsDraw(chart: any) {
        ...
        const yPos = scales.y.getPixelForValue(-1.5);
        ...
        const xPos = scales.x.getPixelForValue(69);
  ...
  const chartOptions = useMemo(() => ({
    ...
    scales: {
      ...
      y: {
        title: {
          display: true,
          text: 'Engellenen Gol Beklentisi'
        },
        min: -10,
        max: 10
      }
