...
interface ForvetFizikselChartProps {
...
export default function ForvetFizikselChart({ 
  forvetData, 
  currentPlayerId, 
  isExpanded, 
  onToggleExpand 
}: ForvetFizikselChartProps) {
  ...
  const referenceLinePlugin = {
    ...
    afterDatasetsDraw(chart: any) {
      ...
      const yPos = scales.y.getPixelForValue(11.25);
  ...
  const chartOptions: ChartOptions<'scatter'> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Genel Fiziksel İstatistik',
        font: {
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Genel Fiziksel İstatistik</h3>
        {onToggleExpand && (
          <button 
            onClick={onToggleExpand}
        ...
        {isExpanded ? (
          <div className="flex flex-col md:flex-row">
            <div className="md:w-1/4 p-2">
              <h4 className="font-semibold text-sm mb-2">Forvet Fiziksel Analizi</h4>
              <p className="text-xs mb-2">Forvet oyuncularının 90 dakika başına sprint ve mesafe değerleri.</p>
              <p className="text-xs">Fiziksel aktivite seviyelerinin karşılaştırması oyuncuların oyun tarzını gösterir.</p>
            </div>
              ...
              <div className="h-[500px] relative">
                ...
                <>
                  ...
                  <div className="absolute ml-12 mt-3 text-left z-10">
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Mesafe Yüksek</div>
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Sprint Düşük</div>
                  </div>
                  
                  {/* Sağ üst köşe - Yeşil */}
                  <div className="absolute right-0 mr-3 mt-3 text-right z-10">
                    <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>Mesafe Yüksek</div>
                    <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>Sprint Yüksek</div>
                  </div>
                  
                  {/* Sol alt köşe - Kırmızı */}
                  <div className="absolute bottom-0 ml-12 mb-12 text-left z-10">
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>Mesafe Düşük</div>
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>Sprint Düşük</div>
                  </div>
                  
                  {/* Sağ alt köşe - Turuncu */}
                  <div className="absolute bottom-0 right-0 mr-3 mb-12 text-right z-10">
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Mesafe Düşük</div>
                    <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Sprint Yüksek</div>
                  </div>
                </>
