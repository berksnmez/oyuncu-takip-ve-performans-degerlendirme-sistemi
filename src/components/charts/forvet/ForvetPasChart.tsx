...
export default function ForvetPasChart({ 
  forvetData, 
  currentPlayerId, 
  isExpanded, 
  onToggleExpand 
  ...
  const referenceLinePlugin = {
    ...
    afterDatasetsDraw(chart: any) {
      ...
      const yPos = scales.y.getPixelForValue(25);
      ...
      if (isExpanded) {
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'left';
        ctx.fillText('25', right + 5, yPos + 4);
      }
      
      // Dikey çizgi için (ortalama değer)
      const xPos = scales.x.getPixelForValue(75);
  ...
  return (
    <div className={`bg-white border rounded-lg shadow-sm transition-all duration-300 overflow-hidden ${
      ...
      <div className="p-2 border-b flex justify-between items-center">
        <h3 className="text-sm font-medium truncate">Pas</h3>
        {onToggleExpand && (
          <button 
      ...
      {isExpanded ? (
        <div className="flex flex-col md:flex-row">
          <div className="md:w-1/4 p-2">
            ...
            <p className="text-xs mb-2">Forvet oyuncularının pas girişim sayıları ve isabet oranları.</p>
            <p className="text-xs">Pas kalitesi ve oyun kurma kapasitesinin karşılaştırması.</p>
          </div>
            ...
            <div className="h-[500px] relative">
              ...
              <>
                ...
                <div className="absolute ml-12 mt-3 text-left z-10">
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Pas Girişim Yüksek</div>
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Pas % Düşük</div>
                </div>
                
                {/* Sağ üst köşe - Yeşil */}
                <div className="absolute right-0 mr-3 mt-3 text-right z-10">
                  <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>Pas Girişim Yüksek</div>
                  <div className="font-bold text-xs" style={{ color: 'rgba(0, 128, 0, 1)' }}>Pas % Yüksek</div>
                </div>
                
                {/* Sol alt köşe - Kırmızı */}
                <div className="absolute bottom-0 ml-12 mb-12 text-left z-10">
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>Pas Girişim Düşük</div>
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 0, 0, 1)' }}>Pas % Düşük</div>
                </div>
                
                {/* Sağ alt köşe - Turuncu */}
                <div className="absolute bottom-0 right-0 mr-3 mb-12 text-right z-10">
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Pas Girişim Düşük</div>
                  <div className="font-bold text-xs" style={{ color: 'rgba(255, 140, 0, 1)' }}>Pas % Yüksek</div>
                </div>
              </>
