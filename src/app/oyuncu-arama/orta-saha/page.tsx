...
export default function OrtaSahaSayfasi() {
  const router = useRouter();
  const [ortaSahaIstatistikleri, setOrtaSahaIstatistikleri] = useState<OrtaSahaIstatistik[]>([]);
  ...
  // Sayfa yüklendiğinde Orta Saha istatistiklerini getir
  useEffect(() => {
    const ortaSahaIstatistikleriniGetir = async () => {
      try {
        setYukleniyor(true);
        const response = await fetch('/api/ortasaha-istatistik');
        const result = await response.json();
        
        if (result.success) {
          const processedData = result.data.map((orta: OrtaSahaIstatistik) => {
            if (!orta.blabla_os) {
              return { ...orta, blabla_os: String(orta.id) };
            }
            return orta;
          });
          
          setOrtaSahaIstatistikleri(processedData);
          
          if (processedData.length > 0) {
            const tumSutunlar = Object.keys(processedData[0]);
            const filtrelenmisStunlar = tumSutunlar.filter(
              sutun => sutun !== 'takim_id' && sutun !== 'oyuncu_id' && sutun !== 'id' && sutun !== 'blabla_os' && (!sutun.endsWith('katsayi') || sutun === 'kalite_sira_katsayi')
            );
        ...
        console.error('Orta Saha istatistikleri getirilirken hata:', error);
        setHata('Orta Saha istatistikleri getirilirken bir hata oluştu');
      } finally {
        setYukleniyor(false);
      }
    };

    ortaSahaIstatistikleriniGetir();
  }, []);
  
  // Takip durumunu güncelle
  useEffect(() => {
    if (ortaSahaIstatistikleri.length > 0) {
      const updatedTakipDurumu: {[key: string]: boolean} = {};
      
      ortaSahaIstatistikleri.forEach((orta: OrtaSahaIstatistik) => {
        if (orta.blabla_os) {
          const blablaOsKey = String(orta.blabla_os).trim();
          updatedTakipDurumu[blablaOsKey] = isInTakipListesi(blablaOsKey, 'orta-saha');
        }
      });
      
      setTakipDurumu(updatedTakipDurumu);
    }
  }, [ortaSahaIstatistikleri, isInTakipListesi, takipListesi]);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return ortaSahaIstatistikleri;

    return [...ortaSahaIstatistikleri].sort((a, b) => {
  ...
  }, [ortaSahaIstatistikleri, sortColumn, sortDirection]);

  const handleTakipListesiToggle = (orta: OrtaSahaIstatistik) => {
    if (!orta.blabla_os) {
      console.error('blabla_os is missing for orta:', orta);
      setNotification({
    ...
    const isCurrentlyInList = isInTakipListesi(blablaOsKey, 'orta-saha');

    if (isCurrentlyInList) {
      removeFromTakipListesi(blablaOsKey, 'orta-saha');
      setTakipDurumu(prev => ({
        ...prev,
        [blablaOsKey]: false
      }));
      setNotification({
        message: `${orta.oyuncu_isim || 'Oyuncu'} takip listesinden çıkarıldı`,
        type: 'warning'
      });
    } else {
      // Tüm orta-saha verilerini takip listesine ekle
      const oyuncuBilgisi = {
        ...orta, // Tüm orta-saha verilerini kopyala
        blabla_os: orta.blabla_os
      };

      console.log("Adding to takip listesi - Orta Saha data:", oyuncuBilgisi);
      console.log("Orta Saha object keys:", Object.keys(oyuncuBilgisi));
      addToTakipListesi(oyuncuBilgisi, 'orta-saha');
      setTakipDurumu(prev => ({
        ...prev,
        [blablaOsKey]: true
      }));
      setNotification({
        message: `${orta.oyuncu_isim || 'Oyuncu'} takip listesine eklendi`,
        type: 'success'
      });
    }
  };
  ...
  const goToTakipListesi = () => {
    router.push('/takip-listesi/orta-saha');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50 p-4">
      <div className="max-w-full mx-auto">
        {/* Pozisyon Navbar */}
        <PozisyonNavbar seciliPozisyon="Orta Saha" />
        
        {/* Modern Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-4">
            🎯 Orta Saha İstatistikleri
          </h1>
          <p className="text-slate-600 text-lg">Oyunun kalbi - Modern istatistiklerle orta saha performansı</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {yukleniyor ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-6 text-slate-600 font-medium">Orta saha verileri yükleniyor...</p>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-2 text-sm text-slate-500 text-center">Veritabanında 'ortasaha_istatistik' tablosu oluşturulduğundan emin olun.</p>
            </div>
          ) : sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium">Henüz orta saha verisi bulunmuyor</p>
              <p className="text-sm text-slate-500">Veritabanına orta saha istatistikleri eklendiğinde burada görünecek</p>
            </div>
          ) : (
            <div>
              ...
              <div className="mb-8 flex items-center justify-between p-6">
                <div className="flex items-center space-x-3">
                  <p className="text-slate-700 font-medium">
                    <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> orta saha bulundu
                  </p>
                </div>
                ...
                <div className="w-full overflow-x-auto" style={{ userSelect: isResizing ? 'none' : 'auto' }}>
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((orta, rowIndex) => {
                        const blablaOsKey = orta.blabla_os ? String(orta.blabla_os).trim() : "";
                        const isInList = blablaOsKey ? takipDurumu[blablaOsKey] || false : false;
                        
                        return (
                          <tr key={orta.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                            ...
                            {sutunlar.map((sutun, index) => {
                              ...
                              const formattedValue = formatValue(orta[sutun], sutun);
                              ...
                              return (
                                ...
                                >
                                  <div 
                                    ...
                                    title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${orta[sutun]}`}
                                  >
                                    {formattedValue}
                                  </div>
                                </td>
                            ...
                            <td className="px-2 py-2 text-right w-20">
                              <button
                                onClick={() => handleTakipListesiToggle(orta)}
