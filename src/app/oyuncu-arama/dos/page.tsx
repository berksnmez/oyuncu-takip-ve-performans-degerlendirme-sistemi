...
export default function DOSSayfasi() {
  const router = useRouter();
  const [dosIstatistikleri, setDOSIstatistikleri] = useState<DOSIstatistik[]>([]);
  ...
  // Sayfa yüklendiğinde DOS istatistiklerini getir
  useEffect(() => {
    const dosIstatistikleriniGetir = async () => {
      try {
        setYukleniyor(true);
        const response = await fetch('/api/dos-istatistik');
        const result = await response.json();
        
        if (result.success) {
          const processedData = result.data.map((dos: DOSIstatistik) => {
            if (!dos.blabla_dos) {
              return { ...dos, blabla_dos: String(dos.id) };
            }
            return dos;
          });
          
          setDOSIstatistikleri(processedData);
        ...
        console.error('DOS istatistikleri getirilirken hata:', error);
        setHata('DOS istatistikleri getirilirken bir hata oluştu');
      } finally {
        setYukleniyor(false);
      }
    };

    dosIstatistikleriniGetir();
  }, []);
  
  // Takip durumunu güncelle
  useEffect(() => {
    if (dosIstatistikleri.length > 0) {
      const updatedTakipDurumu: {[key: string]: boolean} = {};
      
      dosIstatistikleri.forEach((dos: DOSIstatistik) => {
        if (dos.blabla_dos) {
          const blablaDosKey = String(dos.blabla_dos).trim();
          updatedTakipDurumu[blablaDosKey] = isInTakipListesi(blablaDosKey, 'dos');
        }
      });
      
      setTakipDurumu(updatedTakipDurumu);
    }
  }, [dosIstatistikleri, isInTakipListesi, takipListesi]);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return dosIstatistikleri;

    return [...dosIstatistikleri].sort((a, b) => {
  ...
  }, [dosIstatistikleri, sortColumn, sortDirection]);

  const handleTakipListesiToggle = (dos: DOSIstatistik) => {
    if (!dos.blabla_dos) {
      console.error('blabla_dos is missing for dos:', dos);
      setNotification({
    ...
    const isCurrentlyInList = isInTakipListesi(blablaDosKey, 'dos');
    ...
    if (isCurrentlyInList) {
      ...
      setNotification({
        message: `${dos.oyuncu_isim || 'Oyuncu'} takip listesinden çıkarıldı`,
        type: 'warning'
      });
    } else {
      // Tüm DOS verilerini takip listesine ekle
      const oyuncuBilgisi = {
        ...dos, // Tüm DOS verilerini kopyala
        blabla_dos: dos.blabla_dos
      };

      console.log("Adding to takip listesi - DOS data:", oyuncuBilgisi);
      console.log("DOS object keys:", Object.keys(oyuncuBilgisi));
      addToTakipListesi(oyuncuBilgisi, 'dos');
      setTakipDurumu(prev => ({
        ...prev,
        [blablaDosKey]: true
      }));
      setNotification({
        message: `${dos.oyuncu_isim || 'Oyuncu'} takip listesine eklendi`,
        type: 'success'
      });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50 p-4">
      <div className="max-w-full mx-auto">
        {/* Pozisyon Navbar */}
        <PozisyonNavbar seciliPozisyon="DOS" />
        
        {/* Modern Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-4">
            ⚖️ Defansif Orta Saha İstatistikleri
          </h1>
          <p className="text-slate-600 text-lg">Savunmanın dinamosu - Modern istatistiklerle DOS performansı</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {yukleniyor ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-6 text-slate-600 font-medium">DOS verileri yükleniyor...</p>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-2 text-sm text-slate-500 text-center">Veritabanında 'dos_istatistik' tablosu oluşturulduğundan emin olun.</p>
            </div>
          ) : sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium">Henüz DOS verisi bulunmuyor</p>
              <p className="text-sm text-slate-500">Veritabanına DOS istatistikleri eklendiğinde burada görünecek</p>
            </div>
          ) : (
            <div>
              ...
              <div className="mb-8 flex items-center justify-between p-6">
                <div className="flex items-center space-x-3">
                  <p className="text-slate-700 font-medium">
                    <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> DOS bulundu
                  </p>
                </div>
                  ...
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((dos, rowIndex) => {
                        ...
                        const isInList = blablaDosKey ? takipDurumu[blablaDosKey] || false : false;
                        ...
                        return (
                          <tr key={dos.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                            ...
                            {sutunlar.map((sutun, index) => {
                              ...
                              const formattedValue = formatValue(dos[sutun], sutun);
                              ...
                              return (
                                 ...
                                 >
                                  <div 
                                    ...
                                    title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${dos[sutun]}`}
                                  >
                                    {formattedValue}
                                  </div>
                                </td>
                               ...
                               <button
                                 onClick={() => handleTakipListesiToggle(dos)}
