...
export default function StoperSayfasi() {
  const router = useRouter();
  const [stoperIstatistikleri, setStoperIstatistikleri] = useState<StoperIstatistik[]>([]);
  ...
  // Sayfa yüklendiğinde stoper istatistiklerini getir
  useEffect(() => {
    const stoperIstatistikleriniGetir = async () => {
      try {
        setYukleniyor(true);
        const response = await fetch('/api/stoper-istatistik');
        const result = await response.json();
        
        if (result.success) {
          const processedData = result.data.map((stoper: StoperIstatistik) => {
            if (!stoper.blabla_stp) {
              return { ...stoper, blabla_stp: String(stoper.id) };
            }
            return stoper;
          });
          
          setStoperIstatistikleri(processedData);
        ...
        console.error('Stoper istatistikleri getirilirken hata:', error);
        setHata('Stoper istatistikleri getirilirken bir hata oluştu');
      } finally {
        setYukleniyor(false);
      }
    };

    stoperIstatistikleriniGetir();
  }, []);
  
  // Takip durumunu güncelle
  useEffect(() => {
    if (stoperIstatistikleri.length > 0) {
      const updatedTakipDurumu: {[key: string]: boolean} = {};
      
      stoperIstatistikleri.forEach((stoper: StoperIstatistik) => {
        if (stoper.blabla_stp) {
          const blablaStpKey = String(stoper.blabla_stp).trim();
          updatedTakipDurumu[blablaStpKey] = isInTakipListesi(blablaStpKey, 'stoper');
        }
      });
      
      setTakipDurumu(updatedTakipDurumu);
    }
  }, [stoperIstatistikleri, isInTakipListesi, takipListesi]);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return stoperIstatistikleri;

    return [...stoperIstatistikleri].sort((a, b) => {
  ...
  }, [stoperIstatistikleri, sortColumn, sortDirection]);

  // Takip listesi işlemleri
  const handleTakipListesiToggle = (stoper: StoperIstatistik) => {
    ...
    const blablaStpKey = String(stoper.blabla_stp).trim();
    const isCurrentlyInList = takipDurumu[blablaStpKey] || false;
    
    if (isCurrentlyInList) {
      removeFromTakipListesi(blablaStpKey, 'stoper');
      setTakipDurumu(prev => ({ ...prev, [blablaStpKey]: false }));
      setNotification({
        message: 'Stoper takip listenizden çıkarıldı.',
        type: 'success'
      });
    } else {
      const cleanStoper = { ...stoper, blabla_stp: blablaStpKey };
      const result = addToTakipListesi(cleanStoper, 'stoper');
      
      if (result.isDuplicate) {
        setNotification({
          message: 'Bu stoper zaten takip listenizde bulunuyor!',
          type: 'warning'
        });
        setTakipDurumu(prev => ({ ...prev, [blablaStpKey]: true }));
      } else if (result.success) {
        setTakipDurumu(prev => ({ ...prev, [blablaStpKey]: true }));
        setNotification({
          message: 'Stoper takip listenize eklendi.',
          type: 'success'
        });
      } else {
        setNotification({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50 p-4">
      <div className="max-w-full mx-auto">
        {/* Pozisyon Navbar */}
        <PozisyonNavbar seciliPozisyon="Stoper" />
        
        {/* Modern Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-4">
            🧱 Stoper İstatistikleri
          </h1>
          <p className="text-slate-600 text-lg">Savunmanın kalbi - Modern istatistiklerle stoper performansı</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {yukleniyor ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-6 text-slate-600 font-medium">Stoper verileri yükleniyor...</p>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-2 text-sm text-slate-500 text-center">Veritabanında 'stoper_istatistik' tablosu oluşturulduğundan emin olun.</p>
            </div>
          ) : sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium">Henüz stoper verisi bulunmuyor</p>
              <p className="text-sm text-slate-500">Veritabanına stoper istatistikleri eklendiğinde burada görünecek</p>
            </div>
          ) : (
            <div className="p-6">
              ...
              <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <p className="text-slate-700 font-medium">
                    <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> stoper bulundu
                  </p>
                </div>
                  ...
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((stoper, rowIndex) => {
                        ...
                        const isInList = blablaStpKey ? takipDurumu[blablaStpKey] || false : false;
                        ...
                        return (
                          <tr key={stoper.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                            ...
                            <td className="px-2 py-2 text-right">
                              <button
                                onClick={() => handleTakipListesiToggle(stoper)}
