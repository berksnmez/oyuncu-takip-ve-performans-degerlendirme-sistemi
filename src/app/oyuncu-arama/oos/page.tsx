...
export default function OOSSayfasi() {
  const router = useRouter();
  const [oosIstatistikleri, setOOSIstatistikleri] = useState<OOSIstatistik[]>([]);
  ...
  // Sayfa yüklendiğinde OOS istatistiklerini getir
  useEffect(() => {
    const oosIstatistikleriniGetir = async () => {
      try {
        setYukleniyor(true);
        const response = await fetch('/api/ofansifortasaha-istatistik');
        const result = await response.json();
        
        if (result.success) {
          const processedData = result.data.map((oos: OOSIstatistik) => {
            if (!oos.blabla_oos) {
              return { ...oos, blabla_oos: String(oos.id) };
            }
            return oos;
          });
          
          setOOSIstatistikleri(processedData);
          
          if (processedData.length > 0) {
            const tumSutunlar = Object.keys(processedData[0]);
            const filtrelenmisStunlar = tumSutunlar.filter(
              sutun => sutun !== 'takim_id' && sutun !== 'oyuncu_id' && sutun !== 'id' && sutun !== 'blabla_oos' && (!sutun.endsWith('katsayi') || sutun === 'kalite_sira_katsayi')
            );
        ...
        console.error('OOS istatistikleri getirilirken hata:', error);
        setHata('OOS istatistikleri getirilirken bir hata oluştu');
      } finally {
        setYukleniyor(false);
      }
    };

    oosIstatistikleriniGetir();
  }, []);
  
  // Takip durumunu güncelle
  useEffect(() => {
    if (oosIstatistikleri.length > 0) {
      const updatedTakipDurumu: {[key: string]: boolean} = {};
      
      oosIstatistikleri.forEach((oos: OOSIstatistik) => {
        if (oos.blabla_oos) {
          const blablaOosKey = String(oos.blabla_oos).trim();
          updatedTakipDurumu[blablaOosKey] = isInTakipListesi(blablaOosKey, 'oos');
        }
      });
      
      setTakipDurumu(updatedTakipDurumu);
    }
  }, [oosIstatistikleri, isInTakipListesi, takipListesi]);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return oosIstatistikleri;

    return [...oosIstatistikleri].sort((a, b) => {
  ...
  }, [oosIstatistikleri, sortColumn, sortDirection]);

  const handleTakipListesiToggle = (oos: OOSIstatistik) => {
    if (!oos.blabla_oos) {
      console.error('blabla_oos is missing for oos:', oos);
      setNotification({
    ...
    const isCurrentlyInList = isInTakipListesi(blablaOosKey, 'oos');

    if (isCurrentlyInList) {
      removeFromTakipListesi(blablaOosKey, 'oos');
      setTakipDurumu(prev => ({
        ...prev,
        [blablaOosKey]: false
      }));
      setNotification({
        message: `${oos.oyuncu_isim || 'Oyuncu'} takip listesinden çıkarıldı`,
        type: 'warning'
      });
    } else {
      // Tüm OOS verilerini takip listesine ekle
      const oyuncuBilgisi = {
        ...oos, // Tüm OOS verilerini kopyala
        blabla_oos: oos.blabla_oos
      };

      console.log("Adding to takip listesi - OOS data:", oyuncuBilgisi);
      console.log("OOS object keys:", Object.keys(oyuncuBilgisi));
      addToTakipListesi(oyuncuBilgisi, 'oos');
      setTakipDurumu(prev => ({
        ...prev,
        [blablaOosKey]: true
      }));
      setNotification({
        message: `${oos.oyuncu_isim || 'Oyuncu'} takip listesine eklendi`,
        type: 'success'
      });
    }
  };
  ...
  const goToTakipListesi = () => {
    router.push('/takip-listesi/ofansif-orta-saha');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50 p-4">
      <div className="max-w-full mx-auto">
        {/* Pozisyon Navbar */}
        <PozisyonNavbar seciliPozisyon="OOS" />
        
        {/* Modern Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-4">
            ⚽ Ofansif Orta Saha İstatistikleri
          </h1>
          <p className="text-slate-600 text-lg">Saldırının yaratıcısı - Modern istatistiklerle OOS performansı</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {yukleniyor ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-6 text-slate-600 font-medium">OOS verileri yükleniyor...</p>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-2 text-sm text-slate-500 text-center">Veritabanında 'ofansifortasaha_istatistik' tablosu oluşturulduğundan emin olun.</p>
            </div>
          ) : sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium">Henüz OOS verisi bulunmuyor</p>
              <p className="text-sm text-slate-500">Veritabanına OOS istatistikleri eklendiğinde burada görünecek</p>
            </div>
          ) : (
            <div>
              ...
              <div className="mb-8 flex items-center justify-between p-6">
                <div className="flex items-center space-x-3">
                  <p className="text-slate-700 font-medium">
                    <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> OOS bulundu
                  </p>
                </div>
                ...
                <div className="w-full overflow-x-auto" style={{ userSelect: isResizing ? 'none' : 'auto' }}>
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((oos, rowIndex) => {
                        const blablaOosKey = oos.blabla_oos ? String(oos.blabla_oos).trim() : "";
                        const isInList = blablaOosKey ? takipDurumu[blablaOosKey] || false : false;
                        
                        return (
                          <tr key={oos.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                            ...
                            {sutunlar.map((sutun, index) => {
                              ...
                              const formattedValue = formatValue(oos[sutun], sutun);
                              ...
                              return (
                                ...
                                >
                                  <div 
                                    ...
                                    title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${oos[sutun]}`}
                                  >
                                    {formattedValue}
                                  </div>
                                </td>
                            ...
                            <td className="px-2 py-2 text-right w-20">
                              <button
                                onClick={() => handleTakipListesiToggle(oos)}
