...
// Santrafor istatistik veri tipi
...
// Forvet grafik veri tipi (ForvetData ile uyumlu)
...
export default function SantraforDetayPage() {
  const params = useParams();
  const oyuncuId = params.oyuncu_id as string;
  
  const [oyuncuIstatistikleri, setOyuncuIstatistikleri] = useState<SantraforIstatistik | null>(null);
  const [oyuncuGrafik, setOyuncuGrafik] = useState<ForvetGrafik | null>(null);
  const [tumSantraforlar, setTumSantraforlar] = useState<SantraforIstatistik[]>([]);
  ...
  const [karsilastirmaSantrafor, setKarsilastirmaSantrafor] = useState<SantraforIstatistik | null>(null);
    ...
    const verileriYukle = async () => {
      try {
        ...
        const [istatistikResponse, grafikResponse] = await Promise.all([
          fetch(`/api/santrafor-istatistik?oyuncu_id=${oyuncuId}`),
        ...
        // Tüm santrafor verileri (karşılaştırma için)
        const [tumIstatistikResponse, tumGrafikResponse] = await Promise.all([
          fetch('/api/santrafor-istatistik'),
        ...
        if (tumIstatistikResult.success) {
          setTumSantraforlar(tumIstatistikResult.data);
        }
  ...
  // Karşılaştırılabilir santrafor oyuncuları (sadece ST (M))
  const karsilastirabilirSantraforlar = useMemo(() => {
    return tumSantraforlar.filter(santrafor => 
      santrafor.oyuncu_mevkii === 'ST (M)' &&
      santrafor.oyuncu_id.toString() !== oyuncuId
    );
  }, [tumSantraforlar, oyuncuId]);

  // Karşılaştırma santrafor oyuncusu seçme fonksiyonu
  const karsilastirmaSantraforSec = (santrafor: SantraforIstatistik) => {
    setKarsilastirmaSantrafor(santrafor);
    setKarsilastirmaAcik(true);
  };

  // Karşılaştırma analizi yapma fonksiyonu
  const karsilastirmaAnalizi = useMemo(() => {
    if (!oyuncuIstatistikleri || !karsilastirmaSantrafor) return null;

    const oyuncu1 = oyuncuIstatistikleri;
    const oyuncu2 = karsilastirmaSantrafor;
    ...
    const karsilastirmalar = [
      ...
      },
      {
        ...
        oyuncu1Deger: safeNumber(oyuncu1.isbtlsutMB),
        oyuncu2Deger: safeNumber(oyuncu2.isbtlsutMB),
        birim: '/maç',
        yuksekIyi: true
      },
      {
      ...
      {
        kategori: 'Gol Beklentisi',
        oyuncu1Deger: safeNumber(oyuncu1.golbkltnsMB),
        oyuncu2Deger: safeNumber(oyuncu2.golbkltnsMB),
        birim: '/maç',
        yuksekIyi: true
      },
      {
        kategori: 'Kazanılan Hava Topu',
        oyuncu1Deger: safeNumber(oyuncu1.kznhtMB),
        oyuncu2Deger: safeNumber(oyuncu2.kznhtMB),
        birim: '/maç',
        yuksekIyi: true
      }
    ];
  ...
  }, [oyuncuIstatistikleri, karsilastirmaSantrafor]);

  // Lig ortalaması hesaplama fonksiyonu
  const hesaplaLigOrtalamasi = (alan: keyof SantraforIstatistik): number => {
    if (tumSantraforlar.length === 0) return 0;
    ...
    const toplam = tumSantraforlar.reduce((acc, santrafor) => {
      const deger = santrafor[alan];
      return acc + safeNumber(deger);
    }, 0);

    return toplam / tumSantraforlar.length;
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-gray-600">Santrafor Performans Raporu</p>
        </div>

        {/* Santrafor Karşılaştırma Seçimi */}
        <div className="bg-white p-6 rounded-lg shadow-md mb-8">
          <h3 className="text-xl font-semibold text-gray-900 mb-4">Santrafor Karşılaştırması</h3>
          <div className="flex flex-wrap items-center gap-4">
            <label className="text-gray-700 font-medium">Karşılaştırılacak santrafor:</label>
            <select 
              className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent"
              onChange={(e) => {
                const secilenSantrafor = karsilastirabilirSantraforlar.find(s => s.oyuncu_id.toString() === e.target.value);
                if (secilenSantrafor) {
                  karsilastirmaSantraforSec(secilenSantrafor);
                }
              }}
              value={karsilastirmaSantrafor?.oyuncu_id || ''}
            >
              <option value="">Santrafor seçin...</option>
              {karsilastirabilirSantraforlar
                .sort((a, b) => a.oyuncu_isim.localeCompare(b.oyuncu_isim))
                .map(santrafor => (
                  <option key={santrafor.oyuncu_id} value={santrafor.oyuncu_id}>
                    {santrafor.oyuncu_isim} ({santrafor.takim_adi})
                  </option>
                ))
              }
            </select>
          ...
          <div className="mt-2 text-sm text-gray-500">
            * Sadece "ST (M)" mevkiindeki oyuncular karşılaştırılabilir
          </div>
        </div>

        {/* Karşılaştırma Tablosu */}
        {karsilastirmaAcik && karsilastirmaSantrafor && karsilastirmaAnalizi && (
          <div className="bg-white p-6 rounded-lg shadow-md mb-8">
            <h3 className="text-2xl font-semibold text-gray-900 mb-6">Santrafor Karşılaştırması</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {karsilastirmaSantrafor.oyuncu_isim}
                      <div className="text-xs text-gray-400 normal-case">
                        {safeNumber(karsilastirmaSantrafor.oyuncu_sure)}' oynadı
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {karsilastirmaAnalizi.karsilastirmalar.map((karsilastirma, index) => {
                    ...
                    if (karsilastirma.yuksekIyi) {
                      if (karsilastirma.oyuncu1Deger > karsilastirma.oyuncu2Deger) {
                        ...
                        kazanan = karsilastirmaSantrafor.oyuncu_isim;
                        oyuncu2Kazandi = true;
                      } else {
                        kazanan = 'Eşit';
                      }
                  ...
                  <tr className="bg-blue-50">
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-center font-bold ${
                      safeNumber(oyuncuIstatistikleri.performans_skoru) > safeNumber(karsilastirmaSantrafor.performans_skoru) 
                        ? 'text-green-600' : 'text-gray-900'
                    }`}>
                      {safeToFixed(oyuncuIstatistikleri.performans_skoru)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-center font-bold ${
                      safeNumber(karsilastirmaSantrafor.performans_skoru) > safeNumber(oyuncuIstatistikleri.performans_skoru)
                        ? 'text-green-600' : 'text-gray-900'
                    }`}>
                      {safeToFixed(karsilastirmaSantrafor.performans_skoru)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-bold">
                      {safeNumber(oyuncuIstatistikleri.performans_skoru) > safeNumber(karsilastirmaSantrafor.performans_skoru) ? (
                        <span className="text-green-600">🏆</span>
                  ...
                  <tr className="bg-green-50">
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-center font-bold ${
                      safeNumber(oyuncuIstatistikleri.genel_sira) < safeNumber(karsilastirmaSantrafor.genel_sira) 
                        ? 'text-green-600' : 'text-gray-900'
                    }`}>
                      {safeNumber(oyuncuIstatistikleri.genel_sira)}. sıra
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-center font-bold ${
                      safeNumber(karsilastirmaSantrafor.genel_sira) < safeNumber(oyuncuIstatistikleri.genel_sira)
                        ? 'text-green-600' : 'text-gray-900'
                    }`}>
                      {safeNumber(karsilastirmaSantrafor.genel_sira)}. sıra
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-bold">
                      {safeNumber(oyuncuIstatistikleri.genel_sira) < safeNumber(karsilastirmaSantrafor.genel_sira) ? (
                        <span className="text-green-600">🏆</span>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg text-center">
                ...
                <p className="text-sm text-gray-700 mt-1">
                  {oyuncuIstatistikleri.oyuncu_isim} vs {karsilastirmaSantrafor.oyuncu_isim}
                </p>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                <h4 className="font-semibold text-blue-800 mb-2">Tercih</h4>
            ...
            <div className="mt-6 bg-gray-50 p-4 rounded-lg">
              ...
              <div className="text-sm text-gray-700 space-y-2">
                <p>
                  ...
                  {karsilastirmaSantrafor.oyuncu_isim} {safeNumber(karsilastirmaSantrafor.oyuncu_sure)} dakika oynadı.
                </p>
                
                {karsilastirmaAnalizi.oyuncu1Skor > karsilastirmaAnalizi.oyuncu2Skor ? (
                  <p>
                    <strong>{oyuncuIstatistikleri.oyuncu_isim}</strong> özellikle gol üretimi, şut kalitesi ve golcülük etkinliği açısından 
                    {karsilastirmaSantrafor.oyuncu_isim}'den daha başarılı. Bu oyuncu takım için daha güvenilir bir golcü seçenek olabilir.
                  </p>
                ) : karsilastirmaAnalizi.oyuncu2Skor > karsilastirmaAnalizi.oyuncu1Skor ? (
                  <p>
                    <strong>{karsilastirmaSantrafor.oyuncu_isim}</strong> genel performans değerlendirmesinde{' '} 
                  ...
                  <p>
                    Her iki oyuncu da benzer seviyede performans gösteriyor. Seçim yaparken takım oyun tarzı ve 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    <div>
                      ...
                      <ul className="text-blue-600 text-xs space-y-1">
                        {safeNumber(oyuncuIstatistikleri.golMB) > safeNumber(karsilastirmaSantrafor.golMB) && <li>• Daha fazla gol</li>}
                        {safeNumber(oyuncuIstatistikleri.isbtlsutMB) > safeNumber(karsilastirmaSantrafor.isbtlsutMB) && <li>• Daha fazla isabetli şut</li>}
                        {safeNumber(oyuncuIstatistikleri.golculukEtknlk) > safeNumber(karsilastirmaSantrafor.golculukEtknlk) && <li>• Daha iyi golcülük etkinliği</li>}
                        {safeNumber(oyuncuIstatistikleri.pnltszxgMB) > safeNumber(karsilastirmaSantrafor.pnltszxgMB) && <li>• Daha fazla penaltısız xG</li>}
                        {safeNumber(oyuncuIstatistikleri.bsrldrplMB) > safeNumber(karsilastirmaSantrafor.bsrldrplMB) && <li>• Daha fazla başarılı dripling</li>}
                        {safeNumber(oyuncuIstatistikleri.sutdgrlndrme_yzd) > safeNumber(karsilastirmaSantrafor.sutdgrlndrme_yzd) && <li>• Daha yüksek şut doğruluğu</li>}
                        {safeNumber(oyuncuIstatistikleri.kznhtMB) > safeNumber(karsilastirmaSantrafor.kznhtMB) && <li>• Daha fazla kazanılan hava topu</li>}
                      </ul>
                    </div>
                    <div>
                      <p className="text-blue-700 font-medium">{karsilastirmaSantrafor.oyuncu_isim}:</p>
                      <ul className="text-blue-600 text-xs space-y-1">
                        {safeNumber(karsilastirmaSantrafor.golMB) > safeNumber(oyuncuIstatistikleri.golMB) && <li>• Daha fazla gol</li>}
                        {safeNumber(karsilastirmaSantrafor.isbtlsutMB) > safeNumber(oyuncuIstatistikleri.isbtlsutMB) && <li>• Daha fazla isabetli şut</li>}
                        {safeNumber(karsilastirmaSantrafor.golculukEtknlk) > safeNumber(oyuncuIstatistikleri.golculukEtknlk) && <li>• Daha iyi golcülük etkinliği</li>}
                        {safeNumber(karsilastirmaSantrafor.pnltszxgMB) > safeNumber(oyuncuIstatistikleri.pnltszxgMB) && <li>• Daha fazla penaltısız xG</li>}
                        {safeNumber(karsilastirmaSantrafor.bsrldrplMB) > safeNumber(oyuncuIstatistikleri.bsrldrplMB) && <li>• Daha fazla başarılı dripling</li>}
                        {safeNumber(karsilastirmaSantrafor.sutdgrlndrme_yzd) > safeNumber(oyuncuIstatistikleri.sutdgrlndrme_yzd) && <li>• Daha yüksek şut doğruluğu</li>}
                        {safeNumber(karsilastirmaSantrafor.kznhtMB) > safeNumber(oyuncuIstatistikleri.kznhtMB) && <li>• Daha fazla kazanılan hava topu</li>}
                      </ul>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">
                Toplam: {safeNumber(oyuncuIstatistikleri.toplamgol)} gol
              </div>
            </div>

            {/* İsabetli Şut */}
            <div className="text-center">
              <h4 className="text-lg font-medium text-gray-700 mb-2">İsabetli Şut/Maç</h4>
              <div className="text-3xl font-bold text-blue-600 mb-1">
                {safeToFixed(oyuncuIstatistikleri.isbtlsutMB)}
              </div>
              <div className="text-sm text-gray-500">
                Lig Ort: {hesaplaLigOrtalamasi('isbtlsutMB').toFixed(2)}
              </div>
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.isbtlsutMB_katsayi)}/33
              </div>
            </div>

            {/* Golcülük Etkinliği */}
            <div className="text-center">
              <h4 className="text-lg font-medium text-gray-700 mb-2">Golcülük Etkinliği</h4>
              <div className="text-3xl font-bold text-purple-600 mb-1">
                {safeToFixed(oyuncuIstatistikleri.golculukEtknlk)}
              </div>
              <div className="text-sm text-gray-500">
                Lig Ort: {hesaplaLigOrtalamasi('golculukEtknlk').toFixed(2)}
              </div>
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.golculukEtknlk_katsayi)}/33
              </div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.pnltszxgMB_katsayi)}/33
              </div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.bsrldrplMB_katsayi)}/33
              </div>
            </div>

            {/* Şut Doğrulanma */}
            <div className="text-center">
              <h4 className="text-lg font-medium text-gray-700 mb-2">Şut Doğrulama %</h4>
              <div className="text-3xl font-bold text-indigo-600 mb-1">
                {safeNumber(oyuncuIstatistikleri.sutdgrlndrme_yzd)}%
              </div>
              <div className="text-sm text-gray-500">
                Lig Ort: {hesaplaLigOrtalamasi('sutdgrlndrme_yzd').toFixed(1)}%
              </div>
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.sutdgrlndrme_yzd_katsayi)}/33
              </div>
            </div>

            {/* Gol Beklentisi */}
            <div className="text-center">
              <h4 className="text-lg font-medium text-gray-700 mb-2">Gol Beklentisi/Maç</h4>
              <div className="text-3xl font-bold text-yellow-600 mb-1">
                {safeToFixed(oyuncuIstatistikleri.golbkltnsMB)}
              </div>
              <div className="text-sm text-gray-500">
                Lig Ort: {hesaplaLigOrtalamasi('golbkltnsMB').toFixed(2)}
              </div>
              <div className="text-xs text-gray-400">
                Toplam Şut: {safeNumber(oyuncuIstatistikleri.toplamsut)}
              </div>
            </div>

            {/* Kazanılan Hava Topu */}
            <div className="text-center">
              <h4 className="text-lg font-medium text-gray-700 mb-2">Kazanılan Hava Topu/Maç</h4>
              <div className="text-3xl font-bold text-pink-600 mb-1">
                {safeToFixed(oyuncuIstatistikleri.kznhtMB)}
              </div>
              <div className="text-sm text-gray-500">
                Lig Ort: {hesaplaLigOrtalamasi('kznhtMB').toFixed(2)}
              </div>
              <div className="text-xs text-gray-400">
                Sıralama: {safeNumber(oyuncuIstatistikleri.kznhtMB_katsayi)}/33
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="prose max-w-none text-gray-700">
            <p className="mb-4">
              ...
              formasıyla {safeNumber(oyuncuIstatistikleri.oyuncu_sure)} dakika oynayarak ligdeki santraforlar arasında 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <ul className="text-sm text-green-700 space-y-1">
                  ...
                  {safeNumber(oyuncuIstatistikleri.golculukEtknlk) > hesaplaLigOrtalamasi('golculukEtknlk') && (
                    <li>• Yüksek golcülük etkinliği</li>
                  )}
                  {safeNumber(oyuncuIstatistikleri.isbtlsutMB) > hesaplaLigOrtalamasi('isbtlsutMB') && (
                    <li>• İyi şut isabeti</li>
                  )}
                  {safeNumber(oyuncuIstatistikleri.bsrldrplMB) > hesaplaLigOrtalamasi('bsrldrplMB') && (
                    <li>• Başarılı bireysel aksiyonlar</li>
                  )}
                  {safeNumber(oyuncuIstatistikleri.performans_skor_sirasi) <= 10 && (
              ...
              <div className="bg-red-50 p-4 rounded-lg">
                ...
                <ul className="text-sm text-red-700 space-y-1">
                  ...
                  {safeNumber(oyuncuIstatistikleri.isbtlsutMB) < hesaplaLigOrtalamasi('isbtlsutMB') && (
                    <li>• Şut isabetini geliştirmeli</li>
                  )}
                  {safeNumber(oyuncuIstatistikleri.sutdgrlndrme_yzd) < hesaplaLigOrtalamasi('sutdgrlndrme_yzd') && (
                    <li>• Şut doğruluğunu artırmalı</li>
                  )}
                  {safeNumber(oyuncuIstatistikleri.performans_skor_sirasi) > 25 && (
            ...
            <p>
              ...
              33 santrafor arasında <strong>{safeNumber(oyuncuIstatistikleri.performans_skor_sirasi)}. sırada</strong> bulunuyor.
              {safeNumber(oyuncuIstatistikleri.performans_skor_sirasi) <= 10 && 
                " Bu skör, oyuncunun ligdeki en iyi santraforlar arasında yer aldığını gösteriyor."
              }
              {safeNumber(oyuncuIstatistikleri.performans_skor_sirasi) > 25 && 
            ...
            <div className="mt-6 bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-blue-700">
                Bu sezon toplamda <strong>{safeNumber(oyuncuIstatistikleri.toplamgol)} gol</strong> atan ve 
                <strong> {safeNumber(oyuncuIstatistikleri.toplamsut)} şut</strong> çeken oyuncu, 
                maç başına <strong>{safeToFixed(oyuncuIstatistikleri.golMB)} gol</strong> ortalamasıyla performans sergiledi.
                Şut başına gol beklentisi <strong>{safeToFixed(oyuncuIstatistikleri.sutbsnagolbkltns)}</strong> olan oyuncunun
                golcülük verimliliği ligde <strong>{safeNumber(oyuncuIstatistikleri.golculukEtknlk_katsayi)}. sırada</strong> yer alıyor.
              </p>
            </div>
          </div>
        </div>
