...
export async function GET() {
  try {
    // İlk olarak gol_asist tablosundan en çok asist yapan 5 oyuncuyu çek
    const assistQuery = `
      SELECT oyuncu_id, asist 
      FROM gol_asist 
      WHERE asist > 0 
      ORDER BY asist DESC 
      LIMIT 5
    ...
    const assistResult = await query(assistQuery) as any[];
    
    if (assistResult.length === 0) {
      return NextResponse.json({ success: false, message: 'Asist verisi bulunamadı' });
    }
    
    const topAssists = [];
    
    // Her oyuncu için detay bilgilerini çek
    for (const assistData of assistResult) {
      const oyuncuId = assistData.oyuncu_id;
      const asist = assistData.asist;
    ...
    console.error('Asist krallığı verisi çekilirken hata:', error);
    return NextResponse.json(
