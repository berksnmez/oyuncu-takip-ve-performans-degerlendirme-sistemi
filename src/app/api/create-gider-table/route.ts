...
export async function GET() {
  try {
    ...
    await query(`DELETE FROM gider_tablosu`);
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Gider tablosu başarıyla oluşturuldu ve veriler eklendi!' 
    });
  } catch (error: any) {
    console.error('Gider tablosu oluşturulurken hata oluştu:', error);
    return NextResponse.json(
      { success: false, message: 'Gider tablosu oluşturulurken hata oluştu: ' + error.message },
      { status: 500 }
    );
  }
} 