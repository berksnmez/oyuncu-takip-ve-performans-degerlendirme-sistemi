...
interface SantraforItem {
  id: number;
  blabla_snt: string;
  [key: string]: any;
}

// Sütun isimlerinin kısa ve tam karşılıkları - Santrafor özel
const SUTUN_ISIMLERI: { [key: string]: { short: string, full: string } } = {
  ...
  "Gol/90": { short: "Gol/90", full: "Gol/90 Dk" },
  "PH-xG/90": { short: "xG/90", full: "Penaltısız xG/90" },
  "Ast/90": { short: "Ast/90", full: "Asist/90 Dk" },
  "BklAst/90": { short: "xA/90", full: "Beklenen Asist/90" },
  isbtlsutMB: { short: "İ.Şut/M", full: "İsabetli Şut/Maç" },
  sutdgrlndrme_yzd: { short: "Şut.Değ%", full: "Şut Değerlendirme %" },
  golMB: { short: "Gol/M", full: "Gol/Maç" },
  golbkltnsMB: { short: "xG/M", full: "xG/Maç" },
  stdustuxg: { short: "Std.Üst.xG", full: "Standart Üstü xG" },
  sutbsnagolbkltns: { short: "Şut/xG", full: "Şut Başına xG" },
  toplamsut: { short: "T.Şut", full: "Toplam Şut" },
  isbtlsut_yzd: { short: "İ.Şut%", full: "İsabetli Şut %" },
  golculukEtknlk: { short: "G.Etk", full: "Gölcülük Etkinliği" },
  yrtclkEtknlk: { short: "Y.Etk", full: "Yaratıcılık Etkinliği" },
  "Gol/90_katsayi": { short: "G.90.K", full: "Gol/90 Katsayı" },
  "PH-xG/90_katsayi": { short: "xG.90.K", full: "xG/90 Katsayı" },
  "Ast/90_katsayi": { short: "A.90.K", full: "Asist/90 Katsayı" },
  isbtlsutMB_katsayi: { short: "İ.Ş.K", full: "İsabetli Şut/Maç Katsayı" },
  sutdgrlndrme_yzd_katsayi: { short: "Ş.D.K", full: "Şut Değerlendirme % Katsayı" },
  golMB_katsayi: { short: "G.M.K", full: "Gol/Maç Katsayı" },
  stdustuxg_katsayi: { short: "S.Ü.K", full: "Standart Üstü xG Katsayı" },
  golculukEtknlk_katsayi: { short: "G.E.K", full: "Gölcülük Etkinliği Katsayı" },
...
export default function TakipListesiSantrafor() {
  const { getItemsByCategory, removeFromTakipListesi, takipListesi } = useTakipListesi();
  const [santraforlar, setSantraforlar] = useState<SantraforItem[]>([]);
  ...
  useEffect(() => {
    const items = getItemsByCategory('santrafor');
    console.log("Loaded santraforlar:", items);
    
    // Filter and cast items to ensure they all have the blabla_snt field
    const validItems = items
      .filter(item => item.blabla_snt !== undefined)
      .map(item => ({ ...item, blabla_snt: String(item.blabla_snt) })) as SantraforItem[];
      
    setSantraforlar(validItems);
  ...
  const getDefaultColumnWidth = (columnName: string) => {
    ...
    if (columnName.includes('/90')) return 65;
    return 70;
  };
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return santraforlar;
  ...
  }, [santraforlar, sortColumn, sortDirection]);

  // Takip listesinden çıkarma işlemi
  const handleRemoveFromTakipListesi = (santrafor: SantraforItem) => {
    if (!santrafor.blabla_snt) {
      setNotification({
        message: 'Bu santrafor için blabla_snt alanı bulunamadı!',
        type: 'error'
      });
      return;
    }
        ...
        <div className="mb-8">
          ...
          <div className="mt-8 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Takip Edilen Santraforlar</h2>
                <p className="text-slate-600">Gol sanatının en ustası oyuncular</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium mb-2">Henüz takip edilen santrafor bulunmuyor</p>
              <p className="text-slate-500 text-sm mb-6">Favori santrafor oyuncularınızı eklemek için arama sayfasını ziyaret edin</p>
              <Link
               ...
               <div className="mb-8 flex items-center justify-between p-6">
                 <div className="flex items-center space-x-3">
                   <p className="text-slate-700 font-medium">
                     <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> santrafor takip ediliyor
                   </p>
                 </div>
                 <div className="flex items-center space-x-4">
                   {sortedData.length >= 2 && (
                     <Link 
                       href="/takip-listesi/santrafor/karsilastir" 
                        ...
                        <tr key={santrafor.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                          ...
                          {sutunlar.map((sutun, index) => {
                            ...
                            const formatValue = (value: any, columnName: string) => {
                              ...
                              if (sutunAdi.toLowerCase().includes('yüzde') || sutunAdi.includes('%')) {
                            ...
                            const formattedValue = formatValue(santrafor[sutun], sutun);
                            ...
                            return (
                              ...
                              >
                                <div 
                                  ...
                                  title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${santrafor[sutun]}`}
                                >
                                  {formattedValue}
                                </div>
                              </td>
                          ...
                          <td className="px-2 py-2 text-right w-20">
                            <button
                              onClick={() => handleRemoveFromTakipListesi(santrafor)}
