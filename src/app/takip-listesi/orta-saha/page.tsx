...
export default function TakipListesiOrtaSaha() {
  const { getItemsByCategory, removeFromTakipListesi, takipListesi } = useTakipListesi();
  const [ortaSahalar, setOrtaSahalar] = useState<OrtaSahaItem[]>([]);
  ...
  useEffect(() => {
    const items = getItemsByCategory('orta-saha');
    console.log("Loaded ortaSahalar:", items);
    
    // Filter and cast items to ensure they all have the blabla_os field
    const validItems = items
      .filter(item => item.blabla_os !== undefined)
      .map(item => ({ ...item, blabla_os: String(item.blabla_os) })) as OrtaSahaItem[];
      
    setOrtaSahalar(validItems);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return ortaSahalar;
  ...
  }, [ortaSahalar, sortColumn, sortDirection]);

  // Takip listesinden çıkarma işlemi
  const handleRemoveFromTakipListesi = (ortaSaha: OrtaSahaItem) => {
    if (!ortaSaha.blabla_os) {
      setNotification({
        message: 'Bu orta saha için blabla_os alanı bulunamadı!',
        type: 'error'
      });
      return;
    }

    setAnimatingId(ortaSaha.id);
    
    const blablaOsKey = String(ortaSaha.blabla_os).trim();
    removeFromTakipListesi(blablaOsKey, 'orta-saha');
    
    setNotification({
      message: `${ortaSaha.oyuncu_isim} takip listesinden çıkarıldı.`,
      type: 'warning'
    });
        ...
        <div className="mb-8">
          ...
          <div className="mt-8 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Takip Edilen Orta Sahalar</h2>
                <p className="text-slate-600">Favori orta saha oyuncularınızın performans takibi</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium mb-2">Henüz takip edilen orta saha bulunmuyor</p>
              <p className="text-slate-500 text-sm mb-6">Favori orta saha oyuncularınızı eklemek için arama sayfasını ziyaret edin</p>
              <Link
               ...
               <div className="mb-8 flex items-center justify-between p-6">
                 <div className="flex items-center space-x-3">
                   <p className="text-slate-700 font-medium">
                     <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> orta saha takip ediliyor
                   </p>
                 </div>
                 <div className="flex items-center space-x-4">
                   {sortedData.length >= 2 && (
                     <Link 
                       href="/takip-listesi/orta-saha/karsilastir" 
                ...
                <div className="w-full overflow-x-auto" style={{ userSelect: isResizing ? 'none' : 'auto' }}>
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((ortaSaha, rowIndex) => (
                        <tr key={ortaSaha.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                          ...
                          {sutunlar.map((sutun, index) => {
                            ...
                            const formattedValue = formatValue(ortaSaha[sutun], sutun);
                            ...
                            return (
                              ...
                              >
                                <div 
                                  ...
                                  title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${ortaSaha[sutun]}`}
                                >
                                  {formattedValue}
                                </div>
                              </td>
                          ...
                          <td className="px-2 py-2 text-right w-20">
                            <button
                              onClick={() => handleRemoveFromTakipListesi(ortaSaha)}
                              className={`inline-flex items-center justify-center w-9 h-7 rounded-lg text-xs font-medium transition-all duration-300 transform hover:scale-105 ${
                                animatingId === ortaSaha.id ? 'scale-110' : ''
