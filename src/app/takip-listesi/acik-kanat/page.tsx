...
const SUTUN_ISIMLERI: { [key: string]: { short: string, full: string } } = {
  ...
  golbkltnsMB_katsayi: { short: "xG.K", full: "xG/Maç Katsayı" },
...
export default function TakipListesiAcikKanat() {
  const { getItemsByCategory, removeFromTakipListesi, takipListesi } = useTakipListesi();
  const [acikKanatlar, setAcikKanatlar] = useState<AcikKanatItem[]>([]);
  ...
  useEffect(() => {
    const items = getItemsByCategory('acik-kanat');
    console.log("Loaded acikKanatlar:", items);
    
    // Filter and cast items to ensure they all have the blabla_acknt field
    const validItems = items
      .filter(item => item.blabla_acknt !== undefined)
      .map(item => ({ ...item, blabla_acknt: String(item.blabla_acknt) })) as AcikKanatItem[];
      
    setAcikKanatlar(validItems);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return acikKanatlar;
  ...
  }, [acikKanatlar, sortColumn, sortDirection]);

  // Takip listesinden çıkarma işlemi
  const handleRemoveFromTakipListesi = (acikKanat: AcikKanatItem) => {
    if (!acikKanat.blabla_acknt) {
      setNotification({
        message: 'Bu açık kanat için blabla_acknt alanı bulunamadı!',
        type: 'error'
      });
      return;
    }

    setAnimatingId(acikKanat.id);
    
    const blablaAckntKey = String(acikKanat.blabla_acknt).trim();
    removeFromTakipListesi(blablaAckntKey, 'acik-kanat');
    
    setNotification({
      message: `${acikKanat.oyuncu_isim} takip listesinden çıkarıldı.`,
      type: 'warning'
    });
        ...
        <div className="mb-8">
          ...
          <div className="mt-8 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Takip Edilen Açık Kanatlar</h2>
                <p className="text-slate-600">Hızın ve gölün buluştuğu nokta</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium mb-2">Henüz takip edilen açık kanat bulunmuyor</p>
              <p className="text-slate-500 text-sm mb-6">Favori açık kanat oyuncularınızı eklemek için arama sayfasını ziyaret edin</p>
              <Link
               ...
               <div className="mb-8 flex items-center justify-between p-6">
                 <div className="flex items-center space-x-3">
                   <p className="text-slate-700 font-medium">
                     <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> açık kanat takip ediliyor
                   </p>
                 </div>
                 <div className="flex items-center space-x-4">
                   {sortedData.length >= 2 && (
                     <Link 
                       href="/takip-listesi/acik-kanat/karsilastir" 
                ...
                <div className="w-full overflow-x-auto" style={{ userSelect: isResizing ? 'none' : 'auto' }}>
                  <table className="w-full table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-200/50">
                      {sortedData.map((acikKanat, rowIndex) => (
                        <tr key={acikKanat.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                          ...
                          {sutunlar.map((sutun, index) => {
                            ...
                            const formattedValue = formatValue(acikKanat[sutun], sutun);
                            ...
                            return (
                              ...
                              >
                                <div 
                                  ...
                                  title={`${SUTUN_ISIMLERI[sutun]?.full || sutun}: ${acikKanat[sutun]}`}
                                >
                                  {formattedValue}
                                </div>
                              </td>
                          ...
                          <td className="px-2 py-2 text-right w-20">
                            <button
                              onClick={() => handleRemoveFromTakipListesi(acikKanat)}
                              className={`inline-flex items-center justify-center w-9 h-7 rounded-lg text-xs font-medium transition-all duration-300 transform hover:scale-105 ${
                                animatingId === acikKanat.id ? 'scale-110' : ''
