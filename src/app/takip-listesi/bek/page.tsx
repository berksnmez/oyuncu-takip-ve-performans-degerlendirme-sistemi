...
export default function TakipListesiBek() {
  const { getItemsByCategory, removeFromTakipListesi, takipListesi } = useTakipListesi();
  const [bekler, setBekler] = useState<BekItem[]>([]);
  ...
  useEffect(() => {
    const items = getItemsByCategory('bek');
    console.log("Loaded bekler:", items);
    
    // Filter and cast items to ensure they all have the blabla_bek field
    const validItems = items
      .filter(item => item.blabla_bek !== undefined)
      .map(item => ({ ...item, blabla_bek: String(item.blabla_bek) })) as BekItem[];
      
    setBekler(validItems);
  ...
  }, [bekler, sortColumn, sortDirection]);

  // Takip listesinden çıkarma işlemi
  const handleRemoveFromTakipListesi = (bek: BekItem) => {
    if (!bek.blabla_bek) {
      setNotification({
    ...
    removeFromTakipListesi(blablaBekKey, 'bek');
    
    setNotification({
      message: `${bek.oyuncu_isim} takip listesinden çıkarıldı.`,
      type: 'warning'
    });
        ...
        <div className="mb-8">
          ...
          <div className="mt-8 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Takip Edilen Bekler</h2>
                <p className="text-slate-600">Favori beklerinizin performans takibi</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium">Henüz takip edilen bek bulunmuyor</p>
              <p className="text-sm text-slate-500 mt-1">Oyuncu arama sayfasından bek ekleyerek başlayın</p>
              <Link
              ...
              <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <p className="text-slate-700 font-medium">
                    <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> bek takip ediliyor
                  </p>
                </div>
                <div className="flex items-center space-x-4">
                  {sortedData.length >= 2 && (
                    <Link 
                      href="/takip-listesi/bek/karsilastir" 
                        ...
                        return (
                          <tr key={bek.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                            ...
                            <td className="px-2 py-2 text-right">
                              <button
                                onClick={() => handleRemoveFromTakipListesi(bek)}
