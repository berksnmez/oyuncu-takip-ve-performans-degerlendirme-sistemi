...
const SUTUN_ISIMLERI: { [key: string]: { short: string, full: string } } = {
  ...
  sutbsnagolbkltns: { short: "Şut/xG", full: "Şut/xG" },
...
export default function TakipListesiOOS() {
  const { getItemsByCategory, removeFromTakipListesi, takipListesi } = useTakipListesi();
  const [ooslar, setOoslar] = useState<OOSItem[]>([]);
  ...
  useEffect(() => {
    const items = getItemsByCategory('oos');
    console.log("Loaded ooslar:", items);
    
    // Filter and cast items to ensure they all have the blabla_oos field
    const validItems = items
      .filter(item => item.blabla_oos !== undefined)
      .map(item => ({ ...item, blabla_oos: String(item.blabla_oos) })) as OOSItem[];
      
    setOoslar(validItems);
  ...
  const sortedData = React.useMemo(() => {
    if (!sortColumn) return ooslar;

    return [...ooslar].sort((a, b) => {
  ...
  }, [ooslar, sortColumn, sortDirection]);

  // Takip listesinden çıkarma işlemi
  const handleRemoveFromTakipListesi = (oos: OOSItem) => {
    if (!oos.blabla_oos) {
      setNotification({
        message: 'Bu ofansif orta saha için blabla_oos alanı bulunamadı!',
        type: 'error'
      });
      return;
    }
    ...
    setNotification({
      message: `${oos.oyuncu_isim} takip listesinden çıkarıldı.`,
      type: 'warning'
    });
        ...
        <div className="mb-8">
          ...
          <div className="mt-8 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Takip Edilen Ofansif Orta Sahalar</h2>
                <p className="text-slate-600">Favori ofansif orta saha oyuncularınızın performans takibi</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
          {sortedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-600 font-medium mb-2">Henüz takip edilen ofansif orta saha bulunmuyor</p>
              <p className="text-slate-500 text-sm mb-6">Favori ofansif orta saha oyuncularınızı eklemek için arama sayfasını ziyaret edin</p>
              <Link
               ...
               <div className="mb-8 flex items-center justify-between p-6">
                 <div className="flex items-center space-x-3">
                   <p className="text-slate-700 font-medium">
                     <span className="text-2xl font-bold text-emerald-600">{sortedData.length}</span> ofansif orta saha takip ediliyor
                   </p>
                 </div>
                 <div className="flex items-center space-x-4">
                   {sortedData.length >= 2 && (
                     <Link 
                       href="/takip-listesi/ofansif-orta-saha/karsilastir" 
                      ...
                      {sortedData.map((oos, rowIndex) => (
                        <tr key={oos.id} className={`hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-blue-50/50 transition-all duration-200 ${
                          ...
                          <td className="px-2 py-2 text-right w-20">
                            <button
                              onClick={() => handleRemoveFromTakipListesi(oos)}
